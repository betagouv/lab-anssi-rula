...
def test_lister_vide(client: TestClient) -> None:
    r = client.get("/api/retours-bizdev?produit_id=1")
...
def test_import_remplace_les_retours(client: TestClient) -> None:
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv2.encode(), "text/csv")},
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_import_est_isole_par_produit(client: TestClient) -> None:
    ...
    client.post(
        ...
        files={"fichier": ("retours.csv", CSV_VIDE.encode(), "text/csv")},
    )
    assert len(client.get("/api/retours-bizdev?produit_id=1").json()) == 2
    assert client.get("/api/retours-bizdev?produit_id=2").json() == []
...
def test_import_csv_invalide_retourne_400(client: TestClient) -> None:
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv_sans_colonne.encode(), "text/csv")},
    )
...
def test_import_csv_cp1252_avec_bom(client: TestClient) -> None:
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv_bom_cp1252, "text/csv")},
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_import_csv_cp1252_sans_bom(client: TestClient) -> None:
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv_cp1252, "text/csv")},
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
    assert r.json()[0]["verbatim"] == "Retour encodé"
...
def test_ignore_verbatim_vide(client: TestClient) -> None:
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv_avec_vide.encode(), "text/csv")},
    )
    assert r.status_code == 200
    assert len(r.json()) == 1
...
def test_import_nettoie_les_caracteres_de_controle_des_exports(
    client: TestClient,
) -> None:
    csv_avec_controles = (
        "Verbatim,User nom,Rôle du user,Type cible,Source,Lien,Date,Qui ?,Catégorie,Item\n"
    ...
    r = client.post(
        "/api/retours-bizdev/import",
        data=FORMULAIRE,
        files={"fichier": ("retours.csv", csv_avec_controles.encode(), "text/csv")},
    )
